...
public class ItemBuilder extends GenericScreen {
    ...
    protected static NbtEditStyle nbtEditStyle = NbtEditStyle.DESIGN;
    ...
    private Component getButtonTooltip(Tag element, PathNode node) {
        MutableComponent btnTt = Component.empty();
        boolean empty = true;
        if(node != null) {
            btnTt = btnTt.append(Component.nullToEmpty(node.isKey() ? ("Key: "+node.key()) : ("Index: "+node.index())));
            empty = false;
        }
        if(element != null) {
            if(!empty)
                btnTt = btnTt.append("\n");
            btnTt = btnTt.append(Component.nullToEmpty("NBT Type: "+ComponentHelper.formatNbtType(element.getId())));
            empty = false;
        }
        return btnTt;
    }
    ...
    public void createBlankTabNbtEdit(NbtEdit newNbtEdit) {
        ...
        ItemSlotButton btnStyleDesign = new ItemSlotButton(TAB_SIZE, NBT_EDIT_STYLE_ITEMS[0], btn -> {
            nbtEditStyle = NbtEditStyle.DESIGN;
            nbtEditRefreshScreen();
        }).showSlot(false);
        btnStyleDesign.setTooltip(Tooltip.create(Component.nullToEmpty("Style: TODO"))); //TODO rename
        addTabWidgetLocked(tabNum, new PosWidget(btnStyleDesign,0-TAB_SIZE,30+TAB_OFFSET));

        ItemSlotButton btnStyleNbt = new ItemSlotButton(TAB_SIZE, NBT_EDIT_STYLE_ITEMS[1], btn -> {
            nbtEditStyle = NbtEditStyle.NBT;
            nbtEditRefreshScreen();
        }).showSlot(false);
        btnStyleNbt.setTooltip(Tooltip.create(Component.nullToEmpty("Style: NBT")));
        ...
        btnStyleSnbt.setTooltip(Tooltip.create(Component.nullToEmpty("Style: SNBT")));
        addTabWidgetLocked(tabNum, new PosWidget(btnStyleSnbt,0-TAB_SIZE,30+TAB_OFFSET+2*(TAB_SPACING+TAB_SIZE)));

        switch(nbtEditStyle) {
            case DESIGN : btnStyleDesign.active = false; break;
        ...
        {
            EditBox pathTxt = new EditBox(this.font,0,0,(backgroundWidth-5-40)-(5+40+5+15+15)-5,WID_HEIGHT,Component.nullToEmpty(""));
        ...
        if(!foundDesignStyle) {
            ...
            btnStyleDesign.setTooltip(Tooltip.create(Component.empty().append("Style: TODO") //TODO rename
                .append(Component.empty().append("\nNo info found for current path").withStyle(ChatFormatting.YELLOW))));
        }
        ...
        switch(nbtEditStyle) {
            ...
            case SNBT: {
                final String currentVal = BlackMagick.nbtToString(editElement);
                    ...
                    if(widgetCacheTest(WidgetCacheType.NBT_EDIT_SNBT_ADD_BTN)) {
                        ...
                        if(!value.isEmpty()) {
                            ...
                            if(el == null) {
                                ...
                                addBtn.setTooltip(Tooltip.create(Component.nullToEmpty("Invalid element")));
                            }
                                ...
                                addBtn.active = true;
                                addBtn.setTooltip(Tooltip.create(Component.empty().append(
                ...
                {
                    Button w = Button.builder(Component.nullToEmpty("+"), btn -> {
                        Tag el = BlackMagick.nbtFromString(elementTxt.getValue());
                        if(el!=null)
        ...
        //     {
        //         EditBox w = new EditBox(font, x+15, 0, 80, WID_HEIGHT, Component.nullToEmpty(""));
        //         w.setMaxLength(MAX_TEXT_LENGTH);
        //         w.setResponder(value -> {
        //             trySetColorHex(0,value,w);
        //         });
        //         EditBox w2 = new EditBox(font, x+15+100+5, 0, 80, WID_HEIGHT, Component.nullToEmpty(""));
        ...
        //             +BlackMagick.nbtToString(BlackMagick.itemToNbtStorage(editItem))+"},Invisible:true,Pose:{RightArm:[-90f,-90f,0f]}}"));
        //     }
    ...
    public void createBlankTabTextComponent() {
        ...
        //     {
        //         EditBox w = new EditBox(this.font,x+5+40+5,y+5,(240-5-40)-(5+40+5)-5,WID_HEIGHT,Component.nullToEmpty(""));
        //         w.setEditable(false);
        //         w.setMaxLength(MAX_TEXT_LENGTH);
        ...
        //     }
        //     if(textComponentEffectMode == 0 || textComponentEffectMode == 1) {
        //         EditBox w = new EditBox(this.font,x+15-3,y+ROW_TOP,240-36,WID_HEIGHT,Component.nullToEmpty(""));
        //         w.setMaxLength(MAX_TEXT_LENGTH);
        //         w.setResponder(value -> {
        //             updateTextComponentEffect();
        ...
        //         EditBox w2 = new EditBox(this.font,0,0,240-36-80-5,WID_HEIGHT,Component.nullToEmpty(""));
        //         w2.setMaxLength(MAX_TEXT_LENGTH);
        //         w2.setResponder(value -> {
        ...
        //         }
        //         {
        //             EditBox w = new EditBox(font, x+15, 0, 60, WID_HEIGHT, Component.nullToEmpty(""));
        //             w.setMaxLength(MAX_TEXT_LENGTH);
        //             w.setResponder(value -> {
        //                 trySetColorHex(0,value,w);
        //             });
        //             EditBox w2 = new EditBox(font, x+15+100+5, 0, 60, WID_HEIGHT, Component.nullToEmpty(""));
        //             w2.setMaxLength(MAX_TEXT_LENGTH);
        //             w2.setResponder(value -> {
    ...
    protected class RowWidget extends TabWidgetEntry {
        ...
        public void render(GuiGraphics context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            if(displayItem != null) {
                drawItem(context,displayItem,x+ROW_LEFT_SCROLL+2+displayItemXoff,y+2);//to_do draw item for component widget
            }
        ...
        public RowWidgetComponent(String key, boolean component) {
            ...
            if(ComponentHelper.isComplex(pi.type())) {
                ...
                Button.builder(getButtonText(path,startEl), btn -> {
                    createBlankTabNbtEdit(BlackMagick.itemToNbt(selItem),pathNodes);
                ...
                if(BlackMagick.getNbtPath(BlackMagick.itemToNbt(selItem),path) != null) {
                    ...
                    if(tempVal.equals("1b") || tempVal.equals("1") || tempVal.equals("true"))
                        startVal = 2;
                    else
                        startVal = 1;
                }
                ...
                }
                {
                    ...
                    w.setResponder(value -> {
                        if(testPosWidget(2)) {
                            ...
                            if(path.equals("id") && value.isEmpty()) {
                            ...
                            if(!value.equals(startVal) || itemStoneShortcut) {
                                ...
                                if(inpError != null) {
                                    ...
                                    if(
                                        (tempEl != null && BlackMagick.elementsEqual(tempEl,
                                            BlackMagick.getNbtPath(BlackMagick.itemToNbt(BlackMagick.itemFromNbt(BlackMagick.setNbtPath(BlackMagick.itemToNbt(selItem),path,el))),path)
                                        ))
                                        || (
                                            tempEl == null && path.equals("id") && (value.equals("air") || value.equals("minecraft:air"))
                                        )
                                    ) {
                                        unsaved = false;
                ...
                }
                {
                    Button w = Button.builder(Component.nullToEmpty("+"), btn -> {
                        if(testPosWidget(1)) {
                            String inp = ((EditBox)getPosWidget(1)).getValue();
                            if(path.equals("id")) {
                                if(inp.equals("")) {
                ...
                if(tempEl != null) {
                    ...
                    w.active = true;
                    w.setTooltip(Tooltip.create(Component.nullToEmpty("Delete "+(component ? "component" : path))));
                    if(!minecraft.player.getAbilities().instabuild) {
                        w.active = false;
                ...
                ((EditBox)getPosWidget(1)).setValue(startVal);
            }
    ...
    class RowWidgetElement extends RowWidget {
        ...
        public RowWidgetElement(String key) {
            ...
            if(currentEl == null) {
                ...
                addPosWidget(keyBtn, ROW_LEFT_SCROLL, 0);
                {
                    ...
                    w.setResponder(value -> {
                        if(testPosWidget(2)) {
                            ...
                            if(el == null) {
                                ((EditBox)getPosWidget(1)).setTextColor(ERROR_COLOR);
                                setErrorMsg("Invalid element");
                                ((Button)getPosWidget(2)).setTooltip(Tooltip.create(Component.nullToEmpty("Invalid element")));
                            }
                            else if(BlackMagick.elementsEqual(currentEl,el)) {
                                ((EditBox)getPosWidget(1)).setTextColor(LABEL_COLOR);
                ...
                }
                {
                    Button w = Button.builder(Component.nullToEmpty("+"), btn -> {
                        if(testPosWidget(1)) {
                            Tag el = BlackMagick.nbtFromString(((EditBox)getPosWidget(1)).getValue());
                            if(el != null) {
                ...
                ((EditBox)getPosWidget(1)).setValue(currentVal);
            }
        ...
        public static RowWidgetElement customCompoundKey(ItemBuilder context) {
                    ...
                    if(row.testPosWidget(2)) {
                        ...
                        if(!key.isEmpty() || !element.isEmpty()) {
                            if(key.isEmpty()) {
                                ...
                                ((Button)row.getPosWidget(2)).setTooltip(Tooltip.create(Component.nullToEmpty("Invalid key")));
                            }
                            else {
                                Tag el = BlackMagick.nbtFromString(element);
                                if(el == null) {
                                    context.setErrorMsg("Invalid element");
                                    ((Button)row.getPosWidget(2)).setTooltip(Tooltip.create(Component.nullToEmpty("Invalid element")));
                                }
                                else {
                                    ((Button)row.getPosWidget(2)).active = true;
                                    ((Button)row.getPosWidget(2)).setTooltip(Tooltip.create(Component.empty().append(
                                        Component.nullToEmpty("Set key "+BlackMagick.validCompoundKey(key)+" to:\n"+BlackMagick.nbtToString(el)))));
                                }
                            }
                        }
            ...
            }
            {
                Button w = Button.builder(Component.nullToEmpty("+"), btn -> {
                    if(row.testPosWidget(1)) {
                        String key = ((EditBox)row.getPosWidget(0)).getValue();
                        Tag el = BlackMagick.nbtFromString(((EditBox)row.getPosWidget(1)).getValue());
            ...
            ((EditBox)row.getPosWidget(1)).setValue("");
                        ...
                        if(!value.isEmpty()) {
                            ...
                            if(el == null) {
                                ...
                                ((Button)row.getPosWidget(1)).setTooltip(Tooltip.create(Component.empty().append(
                                    Component.nullToEmpty("Append value:\n"+BlackMagick.nbtToString(el)))));
                            }
                        }
                    }
                });
            ...
            addPosWidget(keyBtn, ROW_LEFT_SCROLL, 0);
            {
                if(currentEl == null) {
                    ...
                    txtElement.setResponder(value -> {
                        if(testPosWidget(btnIndex)) {
                            ...
                            if(el == null) {
                                ...
                                setErrorMsg("Invalid element");
                                ((Button)getPosWidget(btnIndex)).setTooltip(Tooltip.create(Component.nullToEmpty("Invalid element")));
                            }
                            else if(BlackMagick.elementsEqual(currentEl,el)) {
                                ...
                                ((Button)getPosWidget(btnIndex)).setTooltip(Tooltip.create(Component.empty().append(
                                    Component.nullToEmpty("Set index "+index+" to:\n"+BlackMagick.nbtToString(el)))));
                            }
                        }
                    });
                    txtElement.setTooltip(Tooltip.create(Component.nullToEmpty("Edit element:\n"+BlackMagick.nbtToString(currentEl))));
                    ...
                    Button btnSet = Button.builder(Component.nullToEmpty("+"), btn -> {
                        Tag el = BlackMagick.nbtFromString(txtElement.getValue());
                        if(el != null) {
                            nbtEditUpdate(fullPath,el);
                        }
                        unsel();
    ...
    public void render(GuiGraphics context, int mouseX, int mouseY, float delta) {
        ...
        if(!tabs[tab].hideTabs()) {
            ...
            if(prevArmorStand)
                InventoryScreen.renderEntityInInventoryFollowsMouse(context, x + playerX, y + playerY, x + playerX + 100, y + playerY + 100, ENTITY_RENDER_SIZE, 0f,
            ...
            if(showBannerPreview && bannerChangePreview != null) {
                if(!bannerShield)
                    InventoryScreen.renderEntityInInventoryFollowsMouse(context,x+backgroundWidth,y,
